'''
Purpose: Provides general utility functions, including random seed setting, log writing, PPI multi-label evaluation metrics, and BFS/DFS subgraph partitioning tools.
Input: Prediction results, labels, probabilities, random seed, PPI edge list, etc.
Output: Evaluation metrics, log text, and graph partition index.
'''
...
def set_seed(seed):
    ...
    if torch.cuda.is_available():
        ...
        torch.cuda.manual_seed_all(seed)  # 多GPU时必须
        torch.backends.cudnn.deterministic = True  # 确保卷积确定性
        torch.backends.cudnn.benchmark = False     # 关闭加速优化
    
    print(f'Success set seed: {seed}')
...
class Metrictor_PPI:
    ...
    def show_result(self, is_print=False, file=None):
        ...
        aupr_entry_1 = np.atleast_2d(self.tru)
        aupr_entry_2 = np.atleast_2d(self.true_prob)
        aupr = np.zeros(7)
        for i in range(7):
            y_true = aupr_entry_1[:, i]
            y_score = aupr_entry_2[:, i]
            if np.sum(y_true) == 0:
                aupr[i] = 0.0
                continue
