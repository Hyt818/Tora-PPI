'''
Purpose: Extract the ego-subgraph from the PPI graph and convert the subgraph information into a sparse mapping structure that is usable by the model.
Input: Training edge indices, number of nodes, hop count, random walk parameters.
Output: Subgraph node mapping, subgraph edge mapping, hop indicator, and merged subgraph structure.
'''
